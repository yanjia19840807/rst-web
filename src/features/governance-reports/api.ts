...
function repositoryFilters(params?: RepositoryListQuery) {
  ...
  if (params) {
    ...
    if (params.customerCountry) search.set('customerCountry', params.customerCountry)
...
function validationWorkflowQuery(params?: ValidationWorkflowQuery) {
  ...
  if (params) {
    ...
    if (params.site) search.set('site', params.site)
    if (params.customerCountry) search.set('customerCountry', params.customerCountry)
    if (params.currentStep) search.set('currentStep', params.currentStep)
    if (params.currentOwner) search.set('currentOwner', params.currentOwner)
    if (params.sizingMonth) search.set('sizingMonth', params.sizingMonth)
...
function benchmarkingFilters(params?: BenchmarkingQuery) {
  const search = new URLSearchParams()
  if (params) {
    if (params.exerciseCode) search.set('exerciseCode', params.exerciseCode)
    if (params.center) search.set('center', params.center)
    if (params.domain) search.set('domain', params.domain)
